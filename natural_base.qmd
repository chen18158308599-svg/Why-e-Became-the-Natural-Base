...
Slide through the animation and watch it happen continuously rather than in fixed steps: as Particle B's remaining distance keeps shrinking by the same proportion, Particle A keeps advancing by the same amount. Geometric decay on one side becomes arithmetic growth on the other — and it's that correspondence, not the numbers themselves, that Napier called a logarithm.
:::
...
Yet mathematicians knew that logarithms could be constructed using many different bases. For example, $\log_{10}(100)=2$ simply means that $10^2=100$.
...
include a graphical demonstration of the relationship between exponential and logarithmic functions, showing how they are inverses of each other.
...
Write $A(x) = L(x) = \int_1^x \frac1t\,dt$ for this same area. The slider below shows the logarithmic behavior directly: the blue area is fixed at $A(x)$, while the red area is the extra area gained when the upper limit is multiplied by $a$ — and it always equals $A(a)$, so $A(ax) = A(x) + A(a)$.
...
These are precisely the two properties that pinned down $L(x)$ back in the previous section.
...
::: {.column width="60%"}
![a spiral galaxy](BAAG_images/A-spiral-galaxy.png){width="463"}
:::
:::::
